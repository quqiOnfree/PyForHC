...
import command
from ui import Ui_reg, Ui_update, Ui_ver, Ui_gamelist, Ui_announcement, Ui_Login, Ui_tellroom_2
...
def init():
    ...
    str_version = "1.0.0.0(massage.beta)"  # 文字版本号
    int_version = 0  # 数字版本号
    # 版本号，需与服务端统一
    update = 0
    must_update = 0
...
class gamelist(QMainWindow, Ui_gamelist):
    def __init__(self):
        super(gamelist, self).__init__()
        self.setupUi(self)
        self.setWindowIcon(QIcon(img))
        self.setWindowTitle("游戏选区")
        s.sendall("get_server_list：{a}：{b}：server_name：game".format(
            a=username, b=des_key).encode("GB2312"))
        self.gameList = receive_if()
        if command.command(self, self.gameList) == 0:
            return
        self.serverlist = eval(self.gameList.split("：")[-1])
    ...
    def closeEvent(self, event):
        global main_win
        main_win.pushButton_4.setEnabled(True)
        self.Close()
...
class MyMainmain(QMainWindow, Ui_tellroom_2):
    ...
    def show_data(self):
        ...
        if command.command(self, user_data) != 0:
            ...
            try:
                self.label_25.setText(username)
                self.label_26.setText(user_data[3])
                self.label_27.setText(user_data[1])
                self.label_28.setText(user_data[2])
                self.label_29.setText(user_data[9])
                self.label_30.setText(user_data[5])
                self.label_31.setText(user_data[4])
                self.label_32.setText(user_data[7])
                self.label_33.setText(user_data[1])
            except:
                pass
